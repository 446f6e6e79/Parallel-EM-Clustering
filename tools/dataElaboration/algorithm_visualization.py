...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--out", dest="output_path", default="data/elaborated/em_visualization.gif",
                        help="Output path (default: data/elaborated/em_visualization.gif)")
    parser.add_argument("--show-initial", action="store_true",
                        help="If set, saves an initial dataset visualization as PNG instead of the GIF.")
    ...
    if args.show_initial:
        print(args.output_path.replace('.gif', '.png'))
        visualize_dataset(args.csv_path, args.output_path.replace('.gif', '.png'))
    else:
        visualize_em(args.csv_path, args.output_path, iterations=args.iterations)