...
- **`src/`**: Directory containing the C source code
  - **`headers/`**: Data structures and function prototypes
...
- **`scripts/`**: Shell scripts for automation
  ...
  - **`multiple_job_generator.sh`**: Generate multiple cluster job scripts
...
- **`docs/`**: LaTeX and media for the report
...
## 2. Requirements
- **MPI (mpicc)**: An MPI implementation (e.g., **OpenMPI** or **MPICH**) with `mpicc` available in your `PATH`.
- **OpenMP**: Needed only for the **hybrid MPI+OpenMP** build.
- **Python 3**: Required for dataset generation and result visualization. Install dependencies with:
  `pip install -r requirements.txt` (see [3.2](#32-generating-the-input-data)).
...
The list of possible parameters for the script is:
...
We provide a job generator that, for each provided dataset, generates job scripts for different node/core combinations and submits them to the cluster:
...
If you want to submit a single job to the cluster, we provide a job script for each available mode (serial/parallel).
...
Performance is evaluated in the provided Jupyter Notebook (`elaborate_data.ipynb`) by loading and aggregating the generated `execution_info.csv` files produced during the runs.  
For further details on the metrics, plots, and the analysis workflow, please refer directly to the notebook.
