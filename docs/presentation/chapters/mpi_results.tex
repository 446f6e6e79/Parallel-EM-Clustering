\section{MPI - Scalability}

% Statistics about the dataset and settings used for scalability experiments
\begin{frame}{Scalability - Experimental Setup}
    \begin{itemize}
        \item The dataset was synthetically generated using our own data generator. This allowed us to create
        datasets of varying sizes while maintaining consistent characteristics.
        \item Composed of:
        \begin{itemize}
            \item Various number of samples, from 156k to 10M
            \item 50 features
            \item 15 clusters
        \end{itemize}
        \item We ignored the initial data loading and scattering, as they are not part of the EM algorithm.
        \item We ignored the convergence criterion, fixing the number of iterations to $100$.
        \item Each configuration was run 5 times, and the average execution time was recorded.
    \end{itemize}
\end{frame}

% Table showing execution times for MPI implementation
\begin{frame}{MPI - Execution Times}
    \begin{table}[h!]
    \centering
...
% Slide showing speedup results for MPI implementation (table and graph)
\begin{frame}{MPI - Speedup Results}
    \begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.2}
    \setlength{\tabcolsep}{4pt}
...
\begin{frame}{MPI - Speedup Results}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{../media/mpi_speedup.png}
...
% Slide showing efficiency results for MPI implementation (table and graph)
\begin{frame}{MPI - Efficiency Results}
    \begin{table}[H]
        \centering
        \renewcommand{\arraystretch}{1.2}
        \setlength{\tabcolsep}{4pt}
...
\begin{frame}{MPI - Efficiency Results}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{../media/mpi_efficiency.png}
...
\begin{frame}{MPI - E-Step vs M-Step Scalability}
    \begin{table}[H]
    \centering
    \scriptsize
...
% Showing the second dataset used to better understand scalability of M-Step
\begin{frame}{MPI - A more in depth look}
    \centering
    To better understand the scalability problem within the M-Step,
    we decided to run additional experiments with a different datasets.
    \\In particular, we increased the number of features \(D = 300\) and clusters \(K = 20 \).
\end{frame}

% Analysis of the other datasets used for scalability experiments
\begin{frame}{MPI - A more in depth look}
    \begin{minipage}{0.48\textwidth}
        \begin{figure}[H]
            \centering
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tabularx}{\textwidth}{c|*{7}{>{\centering\arraybackslash}X}}
            \hline
            \textbf{NP} & 0.033 & 0.066 & 0.131 & 0.262 & 0.524 & 1.049 & 2.097 \\
            \hline
            1  & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 \\
