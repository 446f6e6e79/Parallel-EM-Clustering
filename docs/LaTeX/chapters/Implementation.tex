...
The computation of the Gaussian density function is implemented as a diagonal multivariate Gaussian, which aligns with the data generation process 
Importantly, this simplification does not reduce the computational complexity of the parallel algorithm, since the density must still be computed for each data point and cluster independently. 
Thus, the workload is unaffected by the chosen parallelization strategy.
