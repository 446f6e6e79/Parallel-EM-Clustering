%#TODO: too many repetitions of the same concepts (computation of a single data point is independent from the others, comunication overhead only in M-step, distributing the dataset into chunks,...)
...
The dataset is partitioned into equal chunks and distributed across multiple processing units.
Each processing unit independently performs the E-step on its local data chunk, while the global parameters updates of the M-step are obtained by aggregating the partial results across all units.
Moreover, the clustering assignment step is also parallelized, allowing each processing unit to compute the assignments for its local data chunk based on the final responsibilities.
