...
\begin{comment}
    #TODO: considerare una sola data dependiency che avviene su una stessa riga
    ex: x = x + 1.
    Non è una vera dipendenza, perchè la stessa thread legge e scrive sulla stessa variabile, ma non c'è dipendenza tra thread diverse.
    Considerare solo quelle principali e spiegare come le abbiamo risolte.
\end{comment}
...
\end{tabular}}
\end{table}
%#TODO: add a comment on what are the real dependencies here and how we address to resolve them.
%IN EXAMPLE: the variables that are used by the same thread, are not real dependencies
%M-step
\paragraph{M-step}
\begin{table}[H]
\centering
