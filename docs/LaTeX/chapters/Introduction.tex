...
The EM algorithm is an iterative method used to estimate the parameters of GMMs, allowing us to
identify the underlying clusters in the data. However, the EM algorithm can be computationally intensive,
especially for large datasets, which motivates the need for parallelization to improve efficiency and scalability.
So in this report, we present a parallel implementation of the EM algorithm for clustering, using MPI and OpenMP libraries. 
Then we evaluate the performance of our implementation on various settings, analyzing its scalability, speedup, and efficiency.
In this way, we want to prove the effectiveness of parallelization in accelerating the EM algorithm for clustering large datasets.
