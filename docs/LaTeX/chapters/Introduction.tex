...
\begin{comment}
    An accurate description of the assigned project should be provided,
    including analysis of the sequential algorithm that solves the problem addressed in the project.
    Pseudo-code, examples, graphs, figures, application instances etc. may be provided.
\end{comment}

In this work, we address the problem of clustering large datasets using a parallel implementation of the
...
In this report, we present two parallel implementations of the EM algorithm for clustering.
The first one is based only on MPI, and a second one uses a hybrid approach based on MPI and OpenMP.
We evaluate the performance of our implementation under various settings, analyzing its scalability, speedup, and efficiency.
...
Each cluster is represented by a probability distribution, and in our case, they are modeled as Gaussian distributions.
...
\begin{figure}[H]
    ...
    \end{minipage}\hfill
    \caption{Example of clustering using GMMs with $K=4$ on a 3D dataset.}
    \label{fig:gmm_clusters_3d}
\end{figure}
...
The goal is to estimate the parameters of the Gaussian components and assign data points to these clusters.
...
The responsibilities $\gamma_{ik}$, representing the probability that data point $\boldsymbol{x}_i$ belongs to cluster $k$, are computed as follows:
...
In the following, we provide pseudocode for the overall EM algorithm for GMMs. More in depth pseudocode for each step is provided in the Appendix \ref{app:pseudocode}.
\begin{algorithm}[H]
...
\For{i $= 1 \dots \text{MAX\_ITER}$}
    ...
    \State Compute log-likelihood $\mathcal{L}$ according to \eqref{log_likelihood}
    \If{Converged} 
        \State break 
    \EndIf
\EndFor
