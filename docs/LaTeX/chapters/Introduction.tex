...
In this report, we present two parallel implementation of the EM algorithm for clustering.
The first one based only on MPI, and a second one using a hybrid approach based on MPI and OpenMP.
...
The goal is to estimate, by knowing the parameters of both Gaussian components, to assign data points to these clusters.

A GMMs assumes that each data point is generated from one of $K$ Gaussian components:
