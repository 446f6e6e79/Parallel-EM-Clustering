...
From the results obtained, it is evident that both approaches exhibit excellent efficiency, demostrating weak scalability proprierties and obtaining efficiencies above 0.70 in most configurations.
The pure MPI implementation demonstrates strong scalability, but once the communication became relevant the efficiency starts to drop.
The hybrid approach, on the other hand, manages to maintain a more consistent efficiency across different configurations, particularly in the M-step, where it outperforms the pure MPI implementation in several scenarios.
Unfortunately, we were unable to confront our implementation with other existing solutions on the same hardware, however the results obtained respect the state-of-the-art literature.