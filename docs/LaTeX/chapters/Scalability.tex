...
Each configuration was executed multiple times to ensure the reliability of the results, we report only the average execution times.
...
For the hybrid approach experiments, we explored different combinations of nodes, CPUs per node, and threads per process, as listed in Table \ref{tab:hybrid_combos}.
...
\end{table}
By matching the threads to the number of CPUs, we can compare the hybrid approach directly to the pure MPI implementation.
...
significant, its efficiency starts to drop.
